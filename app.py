...
# Import PyMuPDF as "pymupdf", not the legacy "fitz" name. There's an
# unrelated, abandoned package also called "fitz" on PyPI that causes a
# different, more confusing error if it ever gets installed by mistake.
import pymupdf
...
def apply_annotations_and_stamp(
    ...
    rect = pymupdf.Rect(page.rect.width - 130, 15, page.rect.width - 15, 45)
    ...
    if editor_note.strip():
        note_rect = pymupdf.Rect(20, 50, 250, 85)
        page.draw_rect(
